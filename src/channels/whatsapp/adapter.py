"""WhatsApp adapter - Vision Gemini."""
...
class WhatsAppAdapter(ChannelAdapter):
    ...
    async def _download_image(self, raw: dict) -> tuple:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=25) as client:
                ...
                print("[Download] Status: " + str(r.status_code) + " | msg_id: " + msg_id)
                if r.status_code in (200, 201):
                    ...
                    if b64:
                        ...
                        print("[Download] OK bytes_b64: " + str(len(b64)))
                        return b64, mime
                    else:
                        print("[Download] Sin base64: " + str(resp)[:200])
                else:
                    print("[Download] Error " + str(r.status_code) + ": " + r.text[:200])
    ...
    async def _gemini_analyze(self, image_b64: str, mime: str, caption: str) -> dict:
        ...
        print("[Gemini] Keys disponibles: " + str(len(keys)))
        if not keys:
            print("[Gemini] ERROR: No hay keys configuradas")
            return {"tipo": "otro", "nequi_ok": False, "monto": "", "anime": "no identificado", "prenda": "prenda"}

        prompt = (
            "Analiza esta imagen con maxima precision.\n\n"
            "CASO 1 - COMPROBANTE DE PAGO: Si ves pantalla de transferencia, recibo bancario, Nequi, Bancolombia, valor en pesos, numero de comprobante:\n"
            "TIPO: comprobante\n"
            "NEQUI_OK: si (si el numero " + NEQUI_NUMBER + " aparece en la imagen) o no\n"
            "MONTO: [valor exacto visible, ej: 90000]\n"
            "ANIME: ninguno\n"
            "PRENDA: ninguna\n\n"
            "CASO 2 - PRENDA DE ROPA: Si ves camiseta, buso o polo con estampado:\n"
            "TIPO: prenda\n"
            "NEQUI_OK: no\n"
            "MONTO: ninguno\n"
            ...
            "Responde SOLO las 5 lineas. Sin nada mas."
        )
        body = {
            "contents": [{
                "parts": [
        ...
        for k in keys:
            try:
                gemini_url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=" + k
                print("[Gemini] Llamando con key: " + k[:20] + "...")
                async with httpx.AsyncClient(timeout=25) as client:
                    ...
                    if r.status_code in (200, 201):
                        ...
                        print("[Gemini] 429 rate limit, probando siguiente key")
                        continue
                    else:
                        print("[Gemini] Error " + str(r.status_code) + ": " + r.text[:300])
            except Exception as e:
    ...
    async def analyze_client_image(self, raw: dict) -> dict:
        ...
        payment_words = ["transferencia exitosa", "comprobante", "ya pague", "hice el pago", "realice el pago", "transferi", "te mando el comprobante", "pague", "te envio el pago", "le mande"]
        ...
        if image_b64:
            ...
            return result
        print("[Analyze] No se pudo descargar imagen, fallback por caption")
    ...
    async def send_reply(self, channel_user_id: str, message: OutboundMessage) -> None:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if resp.status_code not in (200, 201):
                print("[WhatsApp] Error: " + str(resp.status_code) + " " + resp.text[:100])
            else:
